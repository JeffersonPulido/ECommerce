...
const PaymentsPage = () => {
    ...
    const onRolUpdated = async (paymentId: string, newState: boolean) => {
        ...
        try {
            await shopApi.put('/admin/payments/', { paymentId, status: newState })
        } catch (error) {
    ...
    const columns: GridColDef[] = [
        {
            ...
            renderCell: ({ row }: GridRenderCellParams) => {
                return (
                    <a href={`/admin/users`} target='_blank' rel='noreferrer'>{`${row.vendorName}`}</a>
                )
            }
            , width: 210
        },
        ...
        { field: 'date', headerName: 'Fecha venta', width: 200 },
        {
            ...
            renderCell: ({ row }: GridRenderCellParams) => {
                return (
                    ...
                    >
                        <MenuItem value='true'>Consignado</MenuItem>
    ...
    return (
        <AdminLayout title={'Dispersion de pagos'} subTitle={'Modulo de pagos a vendedores'} icon={<PeopleOutline />}>
