...
const validGender = ["Hombre", "Mujer", "Unisex"];
const validSizes = ["XS", "S", "M", "L", "XL", "XXL", "N/A"];
const validStatus = ["Nuevo", "Casi Nuevo", "Buen Estado", "Usado"];
interface FormData {
  _id?: string;
  description: string;
  images: string[];
  ...
  status: "Nuevo" | "Casi Nuevo" | "Buen Estado" | "Usado" | string;
}
...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  const onSubmit = async (form: FormData) => {
    ...
    try {
      ...
      if (form._id) {
        setMessageUpdated("Producto actualizado correctamente");
        setTimeout(() => {
          setMessageUpdated("");
          setIsSaving(true);
          ...
          <Grid item xs={12} sm={6}>
            ...
            <TextField
              ...
              {...register("price", {
                required: "Este campo es requerido",
                min: {
                  value: process.env.NEXT_PUBLIC_MINVALUESELL!,
                  message: `El precio minimo es ${process.env
                    .NEXT_PUBLIC_MINVALUESELL!}`,
                },
                validate: {
                  notNegative: (value) =>
                    value >= parseInt(process.env.NEXT_PUBLIC_MINVALUESELL!) ||
