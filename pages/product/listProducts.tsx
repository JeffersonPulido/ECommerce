...
const columns: GridColDef[] = [
    ...
    },
    {
        ...
        renderCell: ({ row }: GridRenderCellParams) => {
            return (
                <NextLink
                    href={`/product/managment/${row.slug}`}
                    passHref
                    legacyBehavior
                >
...
const ListProducts: NextPage<Props> = ({ products }) => {
    ...
    return (
        <ShopLayout
            title={`Mis productos (${products.length})`}
            pageDescription={"Pagina de productos publicados por el usuario"}
        >
            <Typography
                variant="h1"
                component="h1"
            >{`Mis Productos (${products.length})`}</Typography>
...
export const getServerSideProps: GetServerSideProps = async ({
    ...
    if (!session) {
        return {
            redirect: {
                destination: "/auth/login?p=/product/ListProducts",
                permanent: false,
            },
        };
    }
